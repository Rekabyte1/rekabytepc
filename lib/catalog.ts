...
export type Product = {
  ...
  title: string;               // nombre comercial (p.ej. JUGAR 5)
  image: string;               // /pc/jugar5.jpg (pon la tuya en /public)
  priceTransfer: number;
  priceCard: number;
  inStock: boolean;
  gpu: string;                 // ej: rtx-4060 / rtx-4070
  cpu: string;                 // ej: intel-i5-12400f / ryzen-5-5600
  motherboard: string;         // ej: b660 / b760 / b550
  ram: string;                 // ej: 16gb / 32gb
  cabinet: string;             // slug de gabinete (para agrupar modelos)
  cabinetImage: string;        // portada de gabinete
  games: string[];             // slugs de juegos compatibles
  tasks: string[];             // slugs de tareas (trabajar-estudiar, streaming, juegos-2k, etc.)
};
...
export function byGPUFamily(slug: string) {
  // ejemplos: rtx-3060 / rtx-4060 / rtx-4070
