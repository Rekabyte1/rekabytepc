...
/* ────────────────────────────────────────────────────────────
   Página
   ──────────────────────────────────────────────────────────── */
export default function AllGamesIndexPage() {
  const GAMES = useMemo(readGames, []);
  const [res, setRes] = useResolution();

  // Banner por defecto (cámbialo por el que quieras)
  const HERO = "/banners/todos-los-juegos-hero.jpg"; // crea este archivo en /public/banners/
    ...
    <main className="mx-auto max-w-6xl px-4 pb-10">
      ...
      <div className="mb-6 overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900/50">
        <div className="relative h-[280px] sm:h-[340px] md:h-[400px]">
          ...
          />
          {/* degradado sutil para leer textos si pones copy encima */}
      ...
      <section className="mb-6">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          {/* Si más adelante quieres activar 4K, solo descomenta esto
          <button
            type="button"
            disabled
            className="rounded-full px-5 py-3 text-sm font-semibold border bg-neutral-950 text-neutral-400 border-neutral-800 opacity-60 cursor-not-allowed"
          >
            4K (3840 × 2160)
          </button> */}
        </div>

        {/* si más adelante quieres reaccionar al res seleccionado, usa “res”:
        <p className="mt-3 text-center text-xs text-neutral-400">
          Resolución seleccionada: {res === "1080" ? "1080p" : "1440p (2K)"}
        </p> */}
      </section>
